...
def config_dataset(config):
    ...
    config["data_path"] = "/home/featurize/data/" + config["dataset"] + "/"
    if config["dataset"] == "nuswide_21":
        config["data_path"] = "/home/featurize/data/NUS-WIDE/"
    if config["dataset"] in ["nuswide_21_m", "nuswide_81_m"]:
        config["data_path"] = "/dataset/nus_wide_m/"
    if config["dataset"] == "coco":
        config["data_path"] = "/home/featurize/data/COCO_2014/"
...
def compute_result(dataloader, net, device):
    ...
    for img, cls, _ in tqdm(dataloader):
        clses.append(cls)
        emb,  y, cl = net(img.to(device))
        
        bs.append((y).data.cpu())
    return torch.cat(bs).sign(), torch.cat(clses)

def compute_result_lab(dataloader, net, device):
    bs, clses = [], []
    net.eval()
    for img, cls, _ in tqdm(dataloader):
        clses.append(cls)
        features, results, hash  = net(cls.to(device).float())
